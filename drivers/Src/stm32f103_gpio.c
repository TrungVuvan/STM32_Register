...
void GPIO_Init(GPIO_Handle_t *pGPIOHandle)
{
    ...
    if (pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber <= 7)
    {
        ...
        temp |= (pGPIOHandle->GPIO_PinConfig.GPIO_PinCNF << 2); //(pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber * 4 + 2));
				pGPIOHandle->pGPIOx->BRR &= 0x0 << (4 * pGPIOHandle ->GPIO_PinConfig.GPIO_PinNumber);
        pGPIOHandle->pGPIOx->CRL |= temp << (4 * pGPIOHandle ->GPIO_PinConfig.GPIO_PinNumber);
    }
    else
    {
        // Chọn mode Input hoặc Output cho PinNumber
        ...
        // Chọn trạng thái cho PinNumber
        temp |= (pGPIOHandle->GPIO_PinConfig.GPIO_PinCNF << 2);// ((pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber - 8) * 4 + 2));
				pGPIOHandle->pGPIOx->CRH &= 0x0 << (4 * (pGPIOHandle ->GPIO_PinConfig.GPIO_PinNumber - 8));
        pGPIOHandle->pGPIOx->CRH |= temp << (4 * (pGPIOHandle ->GPIO_PinConfig.GPIO_PinNumber - 8));
    }
}
